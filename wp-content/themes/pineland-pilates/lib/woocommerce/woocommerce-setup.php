...
function genesis_sample_products_match_height() {
	...
	wp_enqueue_script( 'pineland-match-height', get_stylesheet_directory_uri() . '/js/jquery.matchHeight.min.js', array( 'jquery' ), CHILD_THEME_VERSION, true );
	wp_add_inline_script( 'pineland-match-height', "jQuery(document).ready( function() { jQuery( '.product .woocommerce-LoopProduct-link').matchHeight(); });" );
...
function genesis_sample_woocommerce_pagination( $args ) {
	...
	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'pineland' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'pineland' ) );
