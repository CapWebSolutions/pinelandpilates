...
function genesis_sample_woocommerce_theme_notice() {
	...
	$notice_html = sprintf( __( 'Please install and activate <a href="https://wordpress.org/plugins/genesis-connect-woocommerce/" target="_blank">Genesis Connect for WooCommerce</a> to <strong>enable WooCommerce support for %s</strong>.', 'pineland' ), esc_html( CHILD_THEME_NAME ) );
	...
	if ( current_user_can( 'install_plugins' ) ) {
		...
		), __( 'install and activate Genesis Connect for WooCommerce', 'pineland' ) );

		$notice_html = sprintf( __( 'Please %s to <strong>enable WooCommerce support for %s</strong>.', 'pineland' ), $install_link, esc_html( CHILD_THEME_NAME ) );
	}

	echo '<div class="notice notice-info is-dismissible pineland-woocommerce-notice"><p>' . $notice_html . '</p></div>';
